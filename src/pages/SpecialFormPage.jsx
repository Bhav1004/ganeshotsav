...
import {
  Store, User, Tag, MapPin, IndianRupee,
  CreditCard, Hash, Ban, ChevronDown, ChevronUp
} from 'lucide-react'
...
const CAT_COLOR    = {
  Shop:     'bg-blue-500',
  VIP:      'bg-yellow-500',
  Bungalow: 'bg-purple-500',
}

export default function SpecialFormPage() {
  const { id }     = useParams()   // 'new' or existing entry id
  ...
  const [step, setStep]         = useState(isNew ? 'entry' : 'donation') // 'entry' | 'donation'
  const [entry, setEntry]       = useState(initEntry)
  const [donation, setDonation] = useState(null)
  const [loading, setLoading]   = useState(!isNew && !initEntry)
  const [submitting, setSubmitting] = useState(false)
  const [error, setError]       = useState('')
  const [showRefuse, setShowRefuse] = useState(false)
  const [refuseNote, setRefuseNote] = useState('')
  const [showUpiNote, setShowUpiNote] = useState(false)
  ...
  // Load existing entry donation if already paid
  useEffect(() => {
    if (!isNew && initEntry) {
      setEntry(initEntry)
  ...
  // Step 1: Save entry details, move to donation step
  const handleSaveEntry = async () => {
    ...
    setSubmitting(false)
    if (err) return setError(err.message || 'Failed to save entry')
    setEntry(data)
    setStep('donation')
  }
  ...
  const handleSubmitDonation = async () => {
    ...
    setSubmitting(false)
    if (err) return setError(err.message || 'Failed to save donation')
    navigate('/thankyou', {
      state: {
        receiptNo: data.receipt_no,
        donation:  data,
        flat:      { flat_number: entry.name, wings: { name: entry.category, buildings: { name: entry.area || '' } } },
      }
    })
  }
  ...
  // ── Already paid — show donation summary ──────────────────────────────────
  ...
  return (
    <div className="min-h-dvh bg-orange-50">
      ...
      {/* Step indicator for new entries */}
      {isNew && (
        <div className="bg-white border-b border-orange-100 px-4 py-2 flex items-center gap-3">
          <div className={`flex items-center gap-1.5 text-sm font-semibold ${step === 'entry' ? 'text-ganesh-orange' : 'text-green-600'}`}>
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs text-white ${step === 'entry' ? 'bg-ganesh-orange' : 'bg-green-500'}`}>
              {step === 'entry' ? '1' : '✓'}
            </span>
            Entry Details
          </div>
          <div className="flex-1 h-0.5 bg-gray-200 rounded"/>
          <div className={`flex items-center gap-1.5 text-sm font-semibold ${step === 'donation' ? 'text-ganesh-orange' : 'text-gray-400'}`}>
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs text-white ${step === 'donation' ? 'bg-ganesh-orange' : 'bg-gray-300'}`}>
              2
            </span>
            Donation
          </div>
      ...
      <div className="px-4 py-4 space-y-4 pb-10">
          ...
          <>
                  ...
                  <button key={cat}
                    ...
                    className={`py-3 rounded-xl font-bold text-sm border-2 transition-all touch-manipulation flex flex-col items-center gap-1 ${
                      entryForm.category === cat
                        ? `${CAT_COLOR[cat]} text-white border-transparent shadow-lg`
            ...
            <div>
              <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1.5">
                <Store size={15}/>
                {entryForm.category === 'Shop' ? 'Shop Name' : entryForm.category === 'VIP' ? 'VIP Name / Title' : 'Bungalow Name'} *
              </label>
            ...
            <div>
              <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1.5">
                <User size={15}/> Owner / Contact Name <span className="text-gray-400 font-normal">(optional)</span>
              </label>
              <input className="input-field"
            ...
            <div>
              <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1.5">
                <MapPin size={15}/> Area / Street <span className="text-gray-400 font-normal">(optional)</span>
              </label>
              <input className="input-field"
        ...
        {step === 'donation' && entry && (
          <>
            {/* Entry summary card */}
            <div className={`rounded-2xl p-4 text-white ${CAT_COLOR[entry.category] || 'bg-ganesh-orange'}`}>
